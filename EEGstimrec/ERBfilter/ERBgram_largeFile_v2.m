function [y,xf,cf,t] = ERBgram_largeFile_v2(x,fs,compress,lowfreq,numchan,smoothcut,resamplefactor)
...
y=nan(size(cf,1),size(x,2));
xf=nan(size(cf,1),size(x,2));
for nf=1:size(cf,1)
    tic;
