...
if manualRej ~=1
    fprintf('Manual artifact rejection switched off...\n ... loading bad channel logfile\n')
    try
        badChans = readtable('wsws_badChans_test.csv', 'ReadVariableNames', false);    
        badChans = table2array(badChans);
    catch
        warning('No bad channel logfile detected, consider performing manual artifact rejection')
        badChans = nan;
    end
    fprintf(' ... loading bad epoch logfile\n')
    try
        badEpochs = readtable('wsws_badEpochs.csv', 'ReadVariableNames', false);    
...
for n=1:length(allfiles)
    ...
    %%% Define epochs
    cfg=[];
    cfg.trialfun            = 'wsws_trialfun_test';
    ...
    if manualRej == 1 
        ...
        dlmwrite('wsws_badChans_test.csv',[str2double(allfiles{n}), str2double(badchannel_labels)'],'delimiter',',','-append');
        dlmwrite('wsws_badEpochs.csv',[str2double(allfiles{n}), badtrialnum'],'delimiter',',','-append');
    else
        ...
        rejected_trials = badEpochs(badEpochs(:,1)==str2num(allfiles{n}),2:end);
        badtrialnum = rejected_trials(~isnan(rejected_trials));
    end
    
    % drop marked epochs (chans will be interpolated)
    cfg = [];
    cfg.trials = 1:length(data.trial);
    cfg.trials(badtrialnum) = [];
    ...
    save([preproc_path filesep 'nlhie_ft_test_SWSNVS' allfiles{n}],'data','event','layout','hdr','rankICA')
